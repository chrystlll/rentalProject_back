package rental.utils;

public class RentalMessage {

	public static String entityNotFound = "Entity {} not found";
	public static String entityNotDeleted = "Entity {} not deleted";
	public static String entityNotSaved = "Entity {} not saved";

	public static String entityDeleted = "Entity {} is deleted";
	public static String entitySaved = "Entity {} is saved";
	public static String entityExist = "Entity {} exists";

	public static String attributeLengthError = "Attribute length for (%s) needs to be (%s)";

	public static String emailInvalid = "Email {} is invalid";
	public static String socialNumberInvalid = "Social number {} is invalid, the current length is {}";
	public static String phoneNumberInvalid = "Phone number {} is invalid, the current length is {}";

}
